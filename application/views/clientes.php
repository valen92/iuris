      ...
      <div class="col s12">
        ...
        <form method="post" name="tercero" id="tercero" action="<?php echo base_url(); ?>Welcome/guardarNuevoCliente"><br>
          ...
          <div class="contiene_tabla2"> 
            <table width="61%" align="center" valign="middle">
              <?php
                if ($datos != 0): 
                foreach ($datos as $fila):
                //Creamos el botón que debe colocar los datos dentro de los campos
                //del formulario que se creará con la función saltar($codConcepto) que le pasamos
                //el codConcepto del concepto
                $boton = array(
                  'name' => 'pasar_edicion',
                  'id' => 'pasar_edicion',
                  'onclick' => "saltar('$fila->identificacionTercero')" //Madmaos la fila como array para quue toe 02 y no 2 etc
                );
              ?>
              <tr>
                <td><?= $fila->identificacionTercero ?></td>
                <td><?= $fila->nombresTercero ?></td>
                <td><a href="<?php echo base_url(); ?>Welcome/verTercero/<?php echo $fila->identificacionTercero ?>" class="Fancy"><?= form_button($boton, 'Editar') ?></a></td>
              </tr>
              <?php
                endforeach;
                endif;
